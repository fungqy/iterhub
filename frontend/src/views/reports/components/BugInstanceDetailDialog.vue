 ...
 *
 * ⚠⚠ 版式:**单栏自上而下,没有左右分栏**(2026-09-19 用户要求,与上面的"改抽屉"同一天但**是两件事**)。
 *   原先左栏放字段与长文本、右栏放「额外那一段」,两栏各 50%、中间一条竖分隔线;
 *   现在那一段**直接接在长文本下方**,抽屉宽度同步减半(1080→540,见 tokens.scss)。
 *   ⇒ 本文件里所有"栏宽"口径都按**单栏**算,别再拿「两栏各 50%」或「原左栏 506px」反推
 *     (那个 506 是**历史值**,它对应的正是现在这一栏的宽度 —— 两者数值接近纯属"减半后正好抵回",
 *      别把它当成"分栏还在"的证据)。
 *   ⇒ `.bugdet-body` 只是竖向堆叠壳,**不是 grid**;`.bugdet-main / .bugdet-aside` 是上下关系。
 *   ⚠ 探针 cdp-drive97(`左右两栏`那一条)守的就是被取消掉的版式,**随本次改动作废**。
 *
 * 故障有两种来源,共用同一个外壳与同一套版式,只有「额外那一段」不同:
 *   - RDM 故障(source='RDM'):字段取自 rdm_issue,额外那一段 = **故障变更记录**(时间线);
 *   - 文档故障(source='DOC'):字段取自 rdm_doc_bug,额外那一段 = **现场截图**。
 *
 ...
 *
 * 关键是判「**这一段该不该存在**」而不是「**现在有没有内容**」(`changelog.length` / `images.length`):
 *   · DOC 的字段与截图清单是并行取的,谁先回来不定 —— 按长度判会让那一段先空后跳
 *     (分栏时抖的是**栏内**;改成堆叠后它落在最下方,抖的只剩容器高度 —— 但"按存在性判"
 *      这条判据本身与版式无关,故一个字不改);
 *   · RDM 无记录、DOC 无截图时,该段应显示「暂无变更记录 / 暂无截图」的空态,
...
<template>
  ...
  >
    ...
    <template v-else-if="isDoc ? !!docDetail : !!rdm">
      <!-- ── 自上而下单栏:① 故障信息(字段 + 长文本) ② 「额外那一段」──
           ② 按来源二选一:RDM 只有变更记录、DOC 只有现场截图,两段互斥。

           ⚠ 2026-09-19 用户要求:去掉左右分栏与那条竖分隔线,把原右栏的内容**移到下方**。
             故外壳由 `grid repeat(2, 1fr)`(各 50%)换成**竖向堆叠**,类名也从 `bugdet-split`
             改为 `bugdet-body` —— 分栏已经没有了,再叫 split 就是留着一个说反话的名字。
             ⛔ 别再改回两栏:那是被明确取消的版式,守它的探针 cdp-drive97 也随之作废。
           ⚠ 两段仍各用 `v-if="!isDoc"` / `v-if="isDoc"` 判"这段该不该存在",**不按 length 增删**
             —— 那是"空态要留着"这条规则(见上方 computed 注释),与分栏无关。
           ⚠ 两段之间的距离取 1rem,与左栏内部各节之间的间距**同一个值**(`gap-4`):
             分隔线去掉后,那一段在视觉上就是"长文本之后的下一节",给它更大的间距反而会
             让人以为这里还留着一道看不见的分界。 -->
      <div class="bugdet-body">
        ...
        <!-- ── 那一段(位置:长文本**下方**,不再分栏):
             RDM 专属 = 故障变更记录(时间线,仅 create/status,节点间标注工作日耗时)──
        ...
        <!-- ── 那一段(位置:长文本**下方**,不再分栏):DOC 专属 = 现场截图 ── -->
        <div v-if="isDoc" class="bugdet-aside flex flex-col gap-2">
          <h6 class="ds-section-title">
            现场截图
...
<style scoped>
/* ── 单栏骨架(2026-09-19 取消左右分栏)──
   自上而下:① .bugdet-main = 故障信息(字段 + 长文本),
             ② .bugdet-aside = 「额外那一段」(RDM 变更记录 / DOC 现场截图)。

   ⛔ 这里原本是 `grid repeat(2, 1fr)` —— 两栏各 50% + 一条竖分隔线,用户 2026-09-17 拍板;
      2026-09-19 用户改口:去掉分栏与分隔线、把右栏内容移到下方。
      ⇒ 原设计里那些"为分栏而生"的规矩(右栏恒定占位、列距必须是 0 好让分隔线落在正中、
        左右栏宽度互相挤动)随分栏一起消失。**这是被新拍板取代,不是回调。**
      ⛔ 别再改回 grid:`border-inline-start` 与两侧 padding 已一并删掉,回退会得到一条
        缺了左右留白、紧贴字段的贴边线。
   ⚠ 间距 1rem = 主栏内各节之间的距离(`gap-4`),取同一个值的原因见模板区那条注释
     (实测 16px,探针 105)。 */
.bugdet-body {
  display: flex;
...
/* ⚠ `min-width: 0` 在单栏下**不再是承重的**(竖向 flex 里没有"两列互相挤"这回事),
   但它拦的是另一件事:内部带 `overflow: auto` 的长文本在 `min-width: auto` 下会拒绝收缩、
   把这一栏顶出容器宽(内容被裁)。留着是零成本的保险,不是分栏留下的残骸。 */
.bugdet-main,
.bugdet-aside {
  min-width: 0;
}
...
.bugdet-grid {
  display: grid;
  /* 阈值 140px:单栏实测宽 496px(见下),排 3 列毫无压力;真正的约束是**窄视口** ——
     抽屉按 44vw 收缩,一路掉到 ~400px 时 140px 仍能排成两列;若提到 190px,
     小屏会退化成单列,16~19 个字段竖成长条。
     ⚠ 栏宽一律按**单栏**口径算。下面两条历史口径都已作废,别再拿来反推:
       · 「95vw / 弹窗 ds-dialog-md 的 760px」—— 2026-09-19 换成抽屉时作废;
       · 「左栏 = (抽屉宽 − 留白) / 2」—— 同一天取消分栏时作废(那时量到 506px)。
       现口径:栏宽 = 抽屉宽 − 内容区左右内边距;抽屉宽 = `--ih-drawer-detail`
       = min(540px, 44vw)(见 tokens.scss / .ds-drawer-detail)。
       实测(2026-09-19,探针 105,1440 视口,两个来源分支共 6 个样本):
       抽屉 540 → 内容区 536 → 单栏 496 → 三轨各 **152px**(152×3 + 20×2 = 496,正好占满)。
       列数仍是 **3 列**、与取消分栏前**一致** —— 抽屉减半(1080→540)正好抵掉"两栏变一栏"
       带来的宽度损失,故字段版式实际没动;`auto-fit` 会自动重排,这里不需要跟着改。
     ⚠ 最长标签「开发负责人」(65px)在各断点下均不折行,dt/dd 也始终同行 ⇒ 140px 仍有余量。 */
...
/* ── 变更记录外框:与「现场截图」的主视图(.docbug-viewer)同款 ──
   ...
   background / border / radius 三项直接对齐那一段,**两种来源**(变更记录 / 现场截图)
   才是同一套容器语言。
...
/* 变更记录整块(时间 + 圆点 + 详情)在框内**水平居中**(用户 2026-09-17 要求;
   ...
   ⚠ 栏头「故障变更记录(N 步)」不跟着居中:它与「故障信息 / 故障原因 / 描述」这些节标题
     同为左对齐的层级标记,要居中的只是时间线这一个**内容块**。 */
.bugdet-timeline {
...
.docbug-preview {
  ...
  /* 同时限高:截图是整屏图(实测有 2772×1458 这类宽幅,也可能出现竖版长图),
     只限宽会让竖版图把下方的元信息行与缩略图带顶出很远。限高后 img 会按比例缩放。
     ⚠ 限高原本的约束是**竖向**的:「预览 + 元信息 + 缩略图带」三行要挤在**右栏**里,
     故收到 34vh/300px。2026-09-19 取消分栏后,这一段落在整列最下方、由抽屉内容区统一滚动,
     那条约束**不再成立** —— 但上限保留,它现在的作用是「别让一张超长竖图把上面 16~19 个字段
     推到几屏之外」,与栏宽无关(改动前先量,别按 rem 猜)。
     ⚠ 宽度口径的历史变更(都已作废,别再引用):ds-dialog-md 的 760px(右栏 ≈ 333px)
     → 95vw 的放大弹窗(右栏 653px)→ 右侧抽屉(右栏 506px)→ **现今单栏 496px**(实测,探针 105)。
...
/* 缩略图源上限 900px、带上一格只有 112px(约八分之一),图必然发虚。
   ⚠⚠ 但**不能就此把 `imageUrl(im, true)` 改成拉原图**:后端 thumb 列可能为 NULL,
   且原图的用途是"点开看细节"(见 .docbug-fullview),不是铺在带上。
   发虚是「格子小」的已知代价,用户 2026-09-17 接受;与分栏无关 ——
   单栏后格子仍是 112×70,这条不随版式变化。 */
.docbug-strip {
  display: flex;
  gap: 0.5rem;
